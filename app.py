...
def data():
    ...
    #cleansing process
    text_clean = re.sub(r'((www\.[^\s]+)|(https?://[^\s]+)|(http?://[^\s]+))|\s|\W|rt|RT|USER|user|xfxfxx[\s]+|@[^s]+|\d|_', ' ', str(text))
...
def input_csv():
    ...
    if request.method == "POST":
        ...
        for line in str(data):
            # cleansing process
            file_clean = re.sub(r'((www\.[^\s]+)|(https?://[^\s]+)|(http?://[^\s]+))|\s|\W|rt|RT|USER|user|xfxfxx[\s]+|@[^s]+|\d|_', ' ', str(data))
            #\s untuk whitespace character [ \t\r\n\f]
            #\d untuk angka
            #\W untuk simbol
        #input data to database
